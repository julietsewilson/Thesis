...
### Optional - remove Bay of Biscay or merge with Celtic
# First change location to character instead of factor (to avoid error when have one less factor)
data$Location <- as.character(data$Location)

#Remove Bay of Biscay samples or merge with Celtic
#data <- subset(data, ! Location == "Bay of Biscay")
# OR
data$Location[data$Location == "Celtic" | data$Location == "Bay of Biscay"] <- "Celtic_Biscay"
...
for(i in 1:samples){ 
  ...
  random_forest <- randomForest(Location ~ ., 
                                ...
                                mtry = 2,
                                importance = TRUE,
                                sampsize = c(33, 33, 33, 33, 33, 33, 33))
...
Assignments$True_location <- factor(Assignments$True_location, levels = c("Norwegian", "Iceland", "Faroes", "North", "West Scotland", "Rockall", "Celtic_Biscay"))

# For merged locations - Reorder Assigned locations to be from north to south
Assignments$Assigned_location <- factor(Assignments$Assigned_location, levels = c("Norwegian", "Iceland", "Faroes", "North", "West Scotland", "Rockall", "Celtic_Biscay"))
...
write.csv(Results_all, file = "SIA_haddock_random_forest_leaveoneout_Celtic_Biscay_sampsize33_v3.csv")
