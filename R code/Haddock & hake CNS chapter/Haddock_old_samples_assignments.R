...
data <- read.csv("Master_CN_haddock.csv", header=TRUE)
...
Results <- as.data.frame(cbind(Norwegian, Iceland, Faroes, North, WScot, Rockall, Celtic, BayBiscay))
Output <- Results*100
...
Total_assigned_Norwegian <- totalAssigned(Output, "Norwegian")
...
write.csv(Output, file = "Results_oldsamples_haddock_assignment_withSuess_Aug22.csv")
