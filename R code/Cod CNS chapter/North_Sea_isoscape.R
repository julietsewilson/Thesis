...
North_sea_data <- read.csv("North_Sea_data_isoscape_v4.csv", header = TRUE)
...
#station_means <- North_sea_data %>% group_by(Station) %>% summarise(mean_C = mean(d13Cc), mean_N = mean(d15N), mean_S = mean(d34S))
#max(station_means$mean_S)
...
v.fit <- fit.variogram(v, vgm(psill=0.68,model ="Lin",range = 1000,nugget = 0.03)) #You can chnage variance aspects of the model or keep it simple 
plot(v, v.fit)

preds = variogramLine(v.fit, maxdist = max(v$dist))

cairo_ps("Cod_NorthSea_Carbon_semivariance_Sept22_viridis_3.eps", height = 10, width = 15)
...
jpeg("Cod_Carbon_isoscape_viridis_Sept22.jpeg")
...
jpeg("Cod_Carbon_variance_viridis_Sept22.jpeg")
...
writeRaster(carbon_raster, "Carbon_isoscape_NorthSea_Sept22", overwrite=TRUE)
writeRaster(carbon_raster_var, "Carbon_variance_NorthSea_Sept22", overwrite=TRUE)
...
v.fit <- fit.variogram(v, vgm(psill=0.68,model ="Lin",range = 1000,nugget = 0.1)) #You can chnage variance aspects of the model or keep it simple 
plot(v, v.fit)

preds = variogramLine(v.fit, maxdist = max(v$dist))

cairo_ps("Cod_NorthSea_Nitrogen_semivariance_Sept22_viridis_v3.eps", height = 10, width = 15)
...
jpeg("Cod_Nitrogen_isoscape_viridis_Sept22.jpeg")
...
jpeg("Cod_Nitrogen_variance_viridis_Sept22.jpeg")
...
writeRaster(nitrogen_raster, "Nitrogen_isoscape_NorthSea_Feb22", overwrite=TRUE)
writeRaster(nitrogen_raster_var, "Nitrogen_variance_NorthSea_Feb22", overwrite=TRUE)
...
#v.fit <- fit.variogram(v, vgm(psill=0.68,model ="Lin",range = 1000,nugget = 0.095)) #You can chnage variance aspects of the model or keep it simple 
#plot(v, v.fit)

v.fit <- fit.variogram(v, vgm(psill=0.5, model ="Exp",range = 240)) #You can chnage variance aspects of the model or keep it simple 
plot(v, v.fit)

#OR
v.fit <- fit.variogram(v, vgm(psill=0.65,model ="Lin",range = 1000,nugget = 0.1)) #You can chnage variance aspects of the model or keep it simple 
plot(v, v.fit)

preds = variogramLine(v.fit, maxdist = max(v$dist))

cairo_ps("Cod_NorthSea_Sulfur_semivariance_Sept22_viridis_v3.eps", height = 10, width = 15)
...
jpeg("Cod_Sulfur_isoscape_viridis_Sept22.jpeg")
...
jpeg("Cod_Sulfur_variance_viridis_Sept22_v2.jpeg")
...
writeRaster(sulfur_raster, "Sulfur_isoscape_NorthSea_Feb22", overwrite=TRUE)
writeRaster(sulfur_raster_var, "Sulfur_variance_NorthSea_Feb22", overwrite=TRUE)

#### To plot isoscapes and variances in one figure

## Carbon isoscape and variance in one figure

jpeg("Carbon_isoscape&variance_viridis_Sept22.jpeg", height=15, width=25, units=c("cm"), res=600)
...
jpeg("Nitrogen_isoscape&variance_viridis.jpeg", height=15, width=25, units=c("cm"), res=600)
...
jpeg("Sulfur_isoscape&variance_viridis.jpeg", height=15, width=25, units=c("cm"), res=600)
...
jpeg("All_isoscape&variance_viridis_Sept22_v2.jpeg", height=25, width=17, units=c("cm"), res=600)
